...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.device = pd.device.get_device()

        # Learning rate and Beta1 for Adam optimizers
        self.lr = args.lr

        # define lr schedulers
        self.exp_lr_scheduler_G = get_scheduler(args)

        # define optimizers
        if args.optimizer == "sgd":
            self.optimizer_G = optim.Momentum(
                learning_rate=self.exp_lr_scheduler_G,
        ...
        self.total_steps = (
            self.max_num_epochs - self.epoch_to_start) * self.steps_per_epoch
        ...
        if args.loss == 'ce':
            ...
            weights = 1 - pd.Tensor(alpha).cuda()
            print(f"Weights = {weights}")
            self._pxl_loss = mIoULoss(
                weight=weights,
                size_average=True,
                n_classes=args.n_class).cuda()
    ...
    def _load_checkpoint(self, ckpt_name='last_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, ckpt_name)):
            ...
            self.optimizer_G.set_state_dict(
                checkpoint['optimizer_G_state_dict'])
            self.exp_lr_scheduler_G.set_state_dict(
            ...
            self.net_G.set_state_dict(
                pd.load(
                    self.args.pretrain)['model_G_state_dict'])
            self.net_G.to(self.device)
            self.net_G.eval()
        else:
    ...
    def _visualize_pred(self):
        pred = pd.argmax(self.G_final_pred, axis=1, keepdim=True)
        pred_vis = pred * 255
        return pred_vis
    ...
    def _update_metric(self):
        ...
        G_pred = pd.argmax(G_pred, axis=1)
    ...
    def _forward_pass(self, batch):
        ...
        img_in1 = batch['A']
        img_in2 = batch['B']
        self.g = self.net_G(img_in1, img_in2)
        self.G_pred = self.g

        if self.multi_scale_infer == "True":
            self.G_final_pred = pd.zeros(
                self.G_pred[-1].shape).to(self.device)
            for pred in self.G_pred:
                if pred.shape[2] != self.G_pred[-1].shape[2]:
                    self.G_final_pred = self.G_final_pred + \
                        F.interpolate(pred, size=self.G_pred[-1].shape[2], mode="nearest")
                else:
    ...
    def _backward_G(self):
        gt = self.batch['L'].astype(pd.float32)
            ...
            for pred in self.G_pred:
                if pred.shape[2] != gt.shape[2]:
                    temp_loss = temp_loss + self.weights[i] * self._pxl_loss(
                        pred, F.interpolate(gt, size=[pred.shape[2], pred.shape[2]], mode="nearest"))
                else:
                    temp_loss = temp_loss + \
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            self.logger.write(
                'lr: %0.7f\n \n' %
                self.optimizer_G.get_lr())
            for self.batch_id, batch in tqdm(
                    enumerate(self.dataloaders['train'], 0), total=total):
