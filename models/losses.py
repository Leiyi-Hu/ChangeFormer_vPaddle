...
def cross_entropy(
    ...
    """
    target = target.astype(pd.int64)
    if target.dim() == 4:
        target = pd.squeeze(target, axis=1)
    ...
    return F.cross_entropy(input=input, label=target, weight=weight,
...
def get_alpha(supervised_loader):
    ...
    for batch in supervised_loader:
        ...
        l_unique = pd.unique(label_batch.data)
        # TODO:
...
def softmax_helper(x):
    ...
    rpt = [1 for _ in range(len(x.shape))]
    rpt[1] = x.shape[1]
...
class FocalLoss(nn.Layer):
    ...
    def forward(self, logit, target):
        ...
        if logit.dim() > 2:
            ...
            logit = logit.transpose([0, 2, 1])  # .contiguous()
            logit = logit.reshape([-1, logit.shape[-1]])
        target = pd.squeeze(target, 1)
        target = target.reshape([-1, 1])

        alpha = self.alpha

        if alpha is None:
            alpha = pd.ones(num_class, 1)
        elif isinstance(alpha, (list, np.ndarray)):
            assert len(alpha) == num_class
            alpha = pd.Tensor(alpha, pd.float32).view(num_class, 1)
        ...
        one_hot_key = pd.zeros((target.shape[0], num_class), pd.float32)
        ...
        alpha = alpha[idx]
        alpha = pd.squeeze(alpha)
        loss = -1 * alpha * pd.pow((1 - pt), gamma) * logpt
...
def to_one_hot_var(tensor, nClasses, requires_grad=False):
    n, h, w = pd.squeeze(tensor, axis=1).shape
    one_hot = tensor.new(n, nClasses, h, w).fill_(0)
    one_hot = one_hot.scatter_(1, tensor.type(pd.int64).view(n, 1, h, w), 1)

    return pd.Tensor(one_hot, requires_grad=requires_grad)
...
class mIoULoss(nn.Layer):
    def __init__(self, weight=None, size_average=True, n_classes=2):
        ...
        self.weights = pd.Tensor(weight)  # Variable(weight)
    ...
    def forward(self, inputs, target, is_target_variable=False):
        ...
        inputs = F.softmax(inputs, axis=1)
