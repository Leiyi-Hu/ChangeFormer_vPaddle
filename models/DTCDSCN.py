...
class Dblock_more_dilate(nn.Layer):
...
class SCSEBlock(nn.Layer):
    def __init__(self, channel, reduction=16):
        ...
        '''self.channel_excitation = nn.Sequential(nn.(channel, int(channel//reduction)),
                                                ...
                                                nn.Sigmoid())'''
        self.channel_excitation = nn.Sequential(nn.Conv2D(channel,
                                                          ...
                                                          bias_attr=False),
                                                nn.ReLU(),
                                                nn.Conv2D(int(channel // reduction),
                                                          channel,
                                                          kernel_size=1,
                                                          stride=1,
                                                          padding=0,
    ...
    def forward(self, x):
        ...
        chn_se = pd.multiply(x, chn_se)
        spa_se = self.spatial_se(x)
        spa_se = pd.multiply(x, spa_se)
        return pd.add(chn_se, 1, spa_se)


class CDNet_model(nn.Layer):
    def __init__(
            self,
            in_channels=3,
            block=SEBasicBlock,
