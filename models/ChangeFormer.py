...
import models.pd_timm
from models.pd_timm import DropPath, to_2tuple
import types
import math
...
class EncoderTransformer(nn.Layer):
    def __init__(
        self,
        img_size=256,
        patch_size=16,
        ...
        # intra-patch encoder
        self.patch_block1 = nn.LayerList([Block(
        ...
        # intra-patch encoder
        self.patch_block2 = nn.LayerList([Block(
        ...
        # intra-patch encoder
        self.patch_block3 = nn.LayerList([Block(
    ...
    def forward_features(self, x):
        ...
        x1 = x1.transpose([0, 2, 1]).reshape([B, embed_dims[1], H1, W1])

        x1 = x1.reshape([x1.shape[0], x1.shape[1], -1]).transpose([0, 2, 1])
        ...
        x1 = x1.transpose([0, 2, 1]).reshape([B, embed_dims[2], H1, W1])
        ...
        x1 = x1.transpose([0, 2, 1]).reshape([B, embed_dims[3], H1, W1])  # +x2
...
class Attention_dec(nn.Layer):
    def __init__(
        ...
        self.task_query = self.create_parameter(
            shape=[1, 48, dim], default_initializer=nn.initializer.Assign(pd.randn([1, 48, dim])))
        self.sr_ratio = sr_ratio
        if sr_ratio > 1:
            self.sr = nn.Conv2D(
                dim, dim, kernel_size=sr_ratio, stride=sr_ratio)
    ...
    def forward(self, x, H, W):
        ...
        # This is because we fix the task parameters to be of a certain
        # dimension, so with varying batch size, we just stack up the same
        # queries to operate on the entire batch
        if B > 1:
            task_q = task_q.unsqueeze(0).repeat(B, 1, 1, 1)
            task_q = task_q.squeeze(1)

        q = self.q(task_q).reshape(
            [B, task_q.shape[1], self.num_heads, C // self.num_heads]).transpose([0, 2, 1, 3])
        ...
        if self.sr_ratio > 1:
            ...
            kv = self.kv(x).reshape(
                [B, -1, 2, self.num_heads, C // self.num_heads]).transpose([2, 0, 3, 1, 4])
        k, v = kv[0], kv[1]
        q = pd.nn.functional.interpolate(q, size=(v.shape[2], v.shape[3]))
...
class convprojection_base(nn.Layer):
    def __init__(self, path=None, **kwargs):
...
# This is the basic ChangeFormer Layer
class ChangeFormerV1(nn.Layer):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False):
        ...
        self.active = pd.nn.Softmax(axis=1)
...
class TDec(nn.Layer):
    ...
    def _transform_inputs(self, inputs):
        ...
        if self.input_transform == 'resize_concat':
            ...
            inputs = pd.concat(upsampled_inputs, axis=1)
    ...
    def forward(self, inputs):
        ...
        _c = self.linear_fuse(pd.concat([_c4, _c3, _c2, _c1], axis=1))
...
class ChangeFormerV2(nn.Layer):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False):
        ...
        self.output_activation = pd.nn.Softmax(axis=1)
...
class EncoderTransformer_x2(nn.Layer):
    def __init__(
        self,
        img_size=256,
        patch_size=3,
        ...
        # Stage-5 (x1/32 scale)
        cur += depths[3]
        self.block5 = nn.LayerList([Block(dim=embed_dims[4],
                                          ...
                                          drop_path=dpr[cur + i],
                                          norm_layer=norm_layer,
                                          sr_ratio=sr_ratios[4]) for i in range(depths[4])])
...
class DecoderTransformer_x2(nn.Layer):
    """
    Transformer Decoder
    """
    ...
    def __init__(
        ...
        self.diff_c4 = conv_diff(
            in_channels=3 * self.embedding_dim,
            out_channels=self.embedding_dim)
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c5_1 = self.linear_c5(c5_1).transpose([0, 2, 1]).reshape(
            [n, -1, c5_1.shape[2], c5_1.shape[3]])
        _c5_2 = self.linear_c5(c5_2).transpose([0, 2, 1]).reshape(
            [n, -1, c5_2.shape[2], c5_2.shape[3]])
        # Difference of features at x1/32 scale
        _c5 = self.diff_c5(pd.concat((_c5_1, _c5_2), dim=1))
        ...
        _c4 = self.diff_c4(
            pd.concat(
                (F.interpolate(
                    ...
                    _c4_1,
                    _c4_2),
                axis=1))  # Difference of features at x1/16 scale
        ...
        _c3 = self.diff_c3(
            pd.concat(
                (F.interpolate(
                    ...
                    _c3_1,
                    _c3_2),
                axis=1))  # Difference of features at x1/8 scale
        ...
        _c2 = self.diff_c2(
            pd.concat(
                (F.interpolate(
                    ...
                    _c2_1,
                    _c2_2),
                axis=1))  # Difference of features at x1/4 scale
