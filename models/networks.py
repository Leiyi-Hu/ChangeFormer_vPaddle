...
from models.Normal import XavierNormal
...
def get_scheduler(args):
    ...
    if args.lr_policy == 'linear':
        ...
        scheduler = lr.LambdaDecay(args.lr, lr_lambda=lambda_rule)
    elif args.lr_policy == 'step':
        step_size = args.max_epochs // 3
        scheduler = lr.StepDecay(args.lr, step_size=step_size, gamma=0.1)
    else:
...
def init_weights(net, init_type='normal', init_gain=0.02):
    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(
                ...
                1):
            if init_type == 'normal':
                init_weight = initializer.Normal(0.0, init_gain)
                init_weight(m.weight)
            elif init_type == 'xavier':
                init_weight = XavierNormal(gain=init_gain)
                init_weight(m.weight)
            elif init_type == 'kaiming':
                init_weight = initializer.KaimingNormal(
                    negative_slope=0, nonlinearity="leaky_relu")
                init_weight(m.weight)
            elif init_type == 'orthogonal':
                init_weight = initializer.Orthogonal(gain=init_gain)
                init_weight(m.weight)
            else:
                raise NotImplementedError(
            ...
            if hasattr(m, 'bias') and m.bias is not None:
                init_bias = initializer.Constant(0.0)
                init_bias(m.bias)
        # BatchNorm Layer's weight is not a matrix; only normal distribution
        # applies.
        elif classname.find('BatchNorm2d') != -1:
            init_weight = initializer.Normal(1.0, init_gain)
            init_weight(m.weigth)
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if len(gpu_ids) > 0:
        net.to(gpu_ids[0])
        if len(gpu_ids) > 1:
            net = pd.DataParallel(net)  # multi-GPUs
...
class ResNet(pd.nn.Layer):
    def __init__(self, input_nc, output_nc,
        ...
        if backbone == 'resnet18':
            self.resnet = models.ResNet18(
                pretrained=True, replace_stride_with_dilation=[
                    False, True, True])  # TODO:use_ssld?
        ...
        self.layer1 = self.resnet.blocks[0: self.resnet.block_depth[0]]
        self.layer2 = self.resnet.blocks[self.resnet.block_depth[0]: (
            self.resnet.block_depth[0] + self.resnet.block_depth[1])]
        self.layer3 = self.resnet.blocks[(self.resnet.block_depth[0] +
                                          self.resnet.block_depth[1]): (self.resnet.block_depth[0] +
                                                                        self.resnet.block_depth[1] +
                                                                        self.resnet.block_depth[2])]
        self.layer4 = self.resnet.blocks[(
            self.resnet.block_depth[0] + self.resnet.block_depth[1] + self.resnet.block_depth[2]):]
    ...
    def forward_single(self, x):
        ...
        if self.resnet_stages_num > 3:
            x_8 = self.layer3(x_8)  # 1/8, in=128, out=256

        if self.resnet_stages_num == 5:
            x_8 = self.layer4(x_8)  # 1/32, in=256, out=512
...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
        ...
        if not self.tokenizer:
            # if not use tokenzier，then downsample the feature map into a
            # certain size
        ...
        if with_pos == 'learned':
            self.pos_embedding = self.create_parameter(
                [1, self.token_len * 2, 32], default_initializer=nn.initializer.Assign(pd.randn([1, self.token_len * 2, 32])))
            # self.pos_embedding = nn.Parameter(pd.randn([1, self.token_len * 2, 32]))
        decoder_pos_size = 256 // 4
        self.with_decoder_pos = with_decoder_pos
        if self.with_decoder_pos == 'learned':
            self.pos_embedding_decoder = self.create_parameter(
                [
                    1,
                    32,
                    decoder_pos_size,
    ...
    def _forward_semantic_tokens(self, x):
        ...
        x = x.reshape([b, c, -1])
        tokens = pd.einsum('bln,bcn->blc', spatial_attention, x)

        return tokens

    def rearrange_x(self, x):
        b, c, h, w = x.shape
    ...
    def transpose_rearrange_x(self, x, b, c, h, w):
        x = x.reshape([b, h, w, c])
    ...
    def _forward_reshape_tokens(self, x):
        ...
        if self.pool_mode == 'max':
            ...
            x = x
        tokens = self.rearrange_x(x)  # rearrange(x, 'b c h w -> b (h w) c')
        return tokens
    ...
    def _forward_transformer_decoder(self, x, m):
        ...
        if self.with_decoder_pos == 'fix':
            ...
            x = x + self.pos_embedding_decoder
        x = self.rearrange_x(x)  # rearrange(x, 'b c h w -> b (h w) c')
        x = self.transformer_decoder(x, m)
        # rearrange(x, 'b (h w) c -> b c h w', h=h)
        x = self.transpose_rearrange_x(x, b, c, h, w)
        return x
    ...
    def forward(self, x1, x2):
        ...
        if self.token_trans:
            self.tokens_ = pd.concat([token1, token2], axis=1)
